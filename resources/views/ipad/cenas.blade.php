...
<body id="cenas-body"
    ...
    <div class="relative z-10 flex flex-row flex-1 h-[calc(100vh-40px)] w-full">
        ...
        <div class="w-1/4 min-w-[280px] max-w-[340px] bg-black/40 backdrop-blur-md border-r border-white/10 p-5 flex flex-col gap-5 text-white overflow-y-auto no-scrollbar">
            
            <div class="flex flex-col gap-2">
                <h2 data-key="search_title_label" class="text-xs font-semibold tracking-wider text-white/60 uppercase">Buscar Cena</h2>
                <div class="relative">
                    <input type="text" id="search-input" oninput="applyFilters()" placeholder="Escribe un platillo o nombre..." 
            ...
            <div>
                <h2 data-key="filter_title" class="text-xs font-semibold tracking-wider text-white/60 uppercase mb-3">Restaurantes / Spots</h2>
                <div class="flex flex-col gap-2.5 max-h-[180px] overflow-y-auto pr-1 no-scrollbar" id="restaurant-filters-container">
                    <label class="flex items-center gap-3 cursor-pointer text-xs group">
                        <input type="radio" name="restaurant-filter" value="all" checked onchange="applyFilters()" class="hidden peer">
                        ...
                        <span data-key="loc_all" class="font-medium text-white group-hover:text-[#C5A059] transition-colors">Todos los restaurantes</span>
                    </label>
                    ...
                    @foreach(['Blue Water', 'Jasmine', 'Bordeaux', 'The grotto', 'Portofino', 'Gazebo', 'Terraza Willy\'s', 'Rose water', 'La taqueria', 'The Market Cafe'] as $rest)
                        <label class="flex items-center gap-3 cursor-pointer text-xs text-white/60 hover:text-white transition-all group">
                            <input type="radio" name="restaurant-filter" value="{{ $rest }}" onchange="applyFilters()" class="hidden peer">
                            <div class="w-4 h-4 rounded-full border border-white/30 group-hover:border-[#C5A059] flex items-center justify-center peer-checked:border-[#C5A059] peer-checked:bg-[#C5A059]">
                                <div class="w-1.5 h-1.5 rounded-full bg-black scale-0 peer-checked:scale-100 transition-all"></div>
                            </div>
            ...
            <div class="flex flex-col gap-4">
                ...
                <div class="space-y-1.5">
                    ...
                    <select id="price-range-filter" onchange="applyFilters()" 
                        ...
                        <option value="2500-5000" data-key="price_mid" class="bg-stone-900">$2,500 MXN - $5,000 MXN</option>
        ...
        <div id="cenas-container" class="flex-1 p-6 overflow-y-auto no-scrollbar flex flex-col gap-4 max-w-4xl mx-auto w-full">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            fetch('/api/hotel/catalog')
                ...
                .catch(err => {
                    ...
                    container.innerHTML = `<div class="text-red-400 text-center py-10 text-xs uppercase">${currentLang === 'en' ? 'Error loading catalog data.' : 'Error al cargar los datos del catálogo.'}</div>`;
                });
        });
        ...
        function renderCenas(cenasFiltradas) {
            ...
            if (!cenasFiltradas || cenasFiltradas.length === 0) {
                container.innerHTML = `<div class="text-white/40 text-center py-12 tracking-wide text-xs uppercase">${translations[currentLang].no_results}</div>`;
                return;
            }

            const t = translations[currentLang];

            // Labels predeterminados para armar la descripción del menú en su respectivo idioma
            ...
            cenasFiltradas.forEach(cena => {
                const descripcionMenu = `<strong>${menu_lbl}:</strong> ${starter_lbl}: ${cena.renderedEntrada || ''} • ${soup_lbl}: ${cena.renderedCrema || ''} • ${main_lbl}: ${cena.renderedPlatoFuerte || ''} • ${dessert_lbl}: ${cena.renderedPostre || ''}.`;
                const restauranteStr = cena.restaurant || 'Hotel';

                const tarjeta = `
                    <div onclick="selectCena('${cena.slug}')" class="bg-black/60 hover:bg-black/70 backdrop-blur-sm border border-white/10 hover:border-[#C5A059]/50 rounded-xl p-5 flex flex-row justify-between items-center transition-all duration-300 cursor-pointer shadow-lg transform active:scale-[0.99] group">
                        <div class="flex-1 pr-6 text-white">
                            <h3 translate="no" class="text-lg font-semibold tracking-wide mb-2 group-hover:text-[#C5A059] transition-colors">${cena.renderedNombre}</h3>
                            <p class="text-xs text-white/70 font-light leading-relaxed mb-3">${descripcionMenu}</p>
                            ...
                            <div class="text-xl font-semibold text-[#C5A059] mt-3 tracking-wide">$${Number(cena.precio).toLocaleString()} <span class="text-xs text-white/50 font-light">MXN</span></div>
                        </div>
                        <div class="w-24 h-24 bg-white/5 border border-white/10 rounded-lg flex flex-col justify-center items-center gap-2 p-2 shrink-0">
                            <i class="fa-solid fa-wine-glass text-2xl text-[#C5A059]"></i>
        ...
        function applyFilters() {
            ...
            let resultados = allCenas.filter(cena => {
                const matchRest = (selectedRestaurant === 'all' || (cena.restaurant && cena.restaurant.toLowerCase() === selectedRestaurant.toLowerCase()));
                ...
                if (selectedPriceRange !== 'all') {
                    ...
                    else if (selectedPriceRange === '2500-5000') matchPrice = (precioCena > 2500 && precioCena <= 5000);
