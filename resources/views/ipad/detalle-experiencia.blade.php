...
<body class="secrets-bg text-stone-200 font-sans min-h-screen flex flex-col justify-between select-none overflow-hidden">
    ...
    <div class="flex-1 w-full max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-12 px-8 items-center py-6 z-10">
        ...
        <div class="md:col-span-6 flex flex-col justify-center items-center w-full relative">
            <div
                ...
                <div id="carouselDots" class="absolute bottom-3 left-0 right-0 flex justify-center gap-2 z-10">
                    @forelse($experiencia->imagenes ?? [] as $foto)
                        <span class="w-2 h-2 rounded-full bg-white/40 cursor-pointer transition-all duration-300"
                              onclick="setSlide({{ $loop->index }})"></span>
                    @empty
                        <span class="w-2 h-2 rounded-full bg-white cursor-pointer transition-all duration-300"
    ...
    <script>
        ...
        if (dots && dots.length > 0) dots[0].classList.add('w-5');
