                    ...
                    <table class="w-full text-sm whitespace-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-sand-200 dark:border-charcoal-500 last:border-0 hover:bg-sand-50 dark:hover:bg-charcoal-500 transition-colors">
                                <td class="px-5 py-3.5 text-gray-900 dark:text-gray-100 font-medium">{{ $ce->Nombre ?? $ce->name }}</td>
                                <td class="px-5 py-3.5 text-gray-500 dark:text-gray-400">{{ $ce->Restaurant ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-gray-900 dark:text-gray-100 text-right font-mono">${{ number_format($ce->Precio ?? $ce->price ?? 0) }}</td>
                                <td class="px-5 py-3.5 text-gray-900 dark:text-gray-100 text-center font-mono">{{ $ce->Numero_Personas ?? $ce->numero_personas ?? '—' }}</td>
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <span class="text-xs px-2 py-0.5 rounded-md border {{ ($ce->Is_Active ?? $ce->is_active) ? 'bg-sapphire-50 dark:bg-sapphire-900/20 text-sapphire-600 dark:text-sapphire-400 border-sapphire-200 dark:border-sapphire-800' : 'bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700' }}">
                                        {{ ($ce->Is_Active ?? $ce->is_active) ? 'Activo' : 'Inactivo' }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5 text-center">
                                    <div class="flex items-center justify-center gap-2 text-gray-400 dark:text-gray-500">
                                        <button onclick='openCenaModal({{ json_encode([
                                            "id" => $ce->Id,
                                            "nombre" => $ce->Nombre ?? $ce->name,
                                            "restaurant" => $ce->Restaurant,
                                            "precio" => $ce->Precio ?? $ce->price,
                                            "numero_personas" => $ce->Numero_Personas ?? $ce->numero_personas,
                                            "activo" => $ce->Is_Active ?? $ce->is_active,
                                        ]) }})'
                                            class="hover:text-gold-500 transition-colors"><i class="fa-solid fa-pen text-xs"></i></button>
                                        @if($ce->Is_Active ?? $ce->is_active)
                                        <form method="POST" action="{{ route('admin.cenas.destroy', $ce->Id) }}" class="inline" onsubmit="return confirm('¿Desactivar esta cena?')">
                                            @csrf @method('DELETE')
                                            <button class="hover:text-red-500 transition-colors"><i class="fa-solid fa-toggle-on text-xs"></i></button>
                                        </form>
                                        @else
                                        <form method="POST" action="{{ route('admin.cenas.activate', $ce->Id) }}" class="inline">
                                            @csrf @method('PATCH')
                            ...
                            @empty
                            <tr>
                                <td colspan="7" class="px-5 py-8 text-center text-gray-400 dark:text-gray-500 text-sm">No hay cenas registradas. ¡Crea la primera!</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <form id="cenaForm" method="POST" enctype="multipart/form-data" class="p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" id="cena_nombre" name="nombre" required
                        ...
                        <div>
                            ...
                            <input type="text" id="cena_restaurant" name="restaurant"
                                class="w-full px-3 py-2.5 rounded-xl border border-sand-200 dark:border-charcoal-500 bg-white dark:bg-charcoal-700 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500/40 focus:border-gold-500 transition">
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="cena_precio" name="precio" step="0.01" min="0" required
                            ...
                            <div>
                                ...
                                <input type="number" id="cena_numero_personas" name="numero_personas" min="1"
                                    class="w-full px-3 py-2.5 rounded-xl border border-sand-200 dark:border-charcoal-500 bg-white dark:bg-charcoal-700 text-gray-900 dark:text-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500/40 focus:border-gold-500 transition">
                            </div>
                        </div>
        ...
        <script>
            function openCenaModal(data) {
                ...
                if (data) {
                    ...
                    document.getElementById('cena_restaurant').value = data.restaurant || '';
                    ...
                    document.getElementById('cena_activo').checked = data.activo === true || data.activo === 1;
                } else {
                    ...
                    document.getElementById('cena_restaurant').value = '';
